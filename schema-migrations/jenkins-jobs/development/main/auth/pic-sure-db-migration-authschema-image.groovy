 pipeline { 
  ...
  stages {
    ...
    stage('Prepare db changes for pic-sure-auth-microapp in the container'){ 
        steps {  
        	sh "docker exec -i authdb_schema_migrations_base_container bash -c \"/picsure-db-migrations/scripts/main/auth/get-auth-schema-from-repo.sh https://github.com/hms-dbmi/pic-sure-auth-microapp.git ${env.PIC_SURE_AUTH_BRANCH_NAME}\"" 
        } 
    } 
