  ...
  stages {
     ...
     stage('Clean and Start the Base Container'){ 
        steps { 
			...
			sh "docker run --name picsuredb_schema_migrations_base_container -v /flyway-configs:/flyway-configs  -d dbmi/pic-sure-db-migrations:${env.DOCKER_IMAGE_RELEASE_TAG}"	 
    ...
    stage('Run DB Migration'){ 
        steps {  
        	 sh '''
        	 	docker exec -i picsuredb_schema_migrations_base_container bash -c \"/picsure-db-migrations/scripts/custom/picsure/picsure-custom-migration-orchestrator.sh\"
			''' 
